...
filebot -rename --action copy $smovies\/* --format "$dmovies/{n}\ ({y})/{n}\ ({y})" -non-strict | tee logs/filebot1.txt
filebot -rename --action copy "$stv"\/* --format "$dtv/{n}\ ({y})/Season {s}/{n}-{s00e00}-{t} " -non-strict | tee logs/filebot2.txt

# Use rsync to transfer from $d to the NAS

d2=/mnt/nas-multimedia/jesse-videos

rsync -avz --progress $dmovies $d2 | tee logs/rsync1.txt
rsync -az -vvv --progress --protect-args /media/jesse/boa/Videos/TV\ Shows $d2 | tee logs/rsync2.txt
